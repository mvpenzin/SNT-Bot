...
import { Activity, Clock, ShieldCheck, AlertCircle, RefreshCw } from "lucide-react";
  ...
  return (
    <Layout>
      <div className="flex flex-col gap-2">
        ...
        <p className="text-muted-foreground">Overview of your bot's health and recent activity.</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <CardContent>
            {isLoadingStatus ? (
              ...
              <div className="flex items-center gap-2">
                <div className={`w-3 h-3 rounded-full ${status?.status === 'running' ? 'bg-emerald-500 animate-pulse' : 'bg-red-500'}`} />
                <div className="text-2xl font-bold capitalize">{status?.status || 'Unknown'}</div>
              </div>
            )}
            <p className="text-xs text-muted-foreground mt-2">
              Last checked: {status?.lastCheck ? formatDistanceToNow(new Date(status.lastCheck), { addSuffix: true }) : '...'}
            </p>
          </CardContent>
        </Card>
          ...
          <CardContent>
            {isLoadingStatus ? (
              ...
              <div className="text-2xl font-bold">
                {status ? Math.floor(status.uptime / 3600) : 0}h {status ? Math.floor((status.uptime % 3600) / 60) : 0}m
              </div>
            )}
        ...
        <Card className="glass-panel border-l-4 border-l-amber-500">
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Health Check</CardTitle>
            <ShieldCheck className="h-4 w-4 text-amber-500" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">Healthy</div>
            <p className="text-xs text-muted-foreground mt-2">Database connection active</p>
          </CardContent>
        </Card>
      </div>
          ...
          {errorLogs.length > 0 ? (
            <div className="space-y-3">
              {errorLogs.slice(0, 3).map((log) => (
                <div key={log.id} className="bg-red-950/20 border border-red-500/20 p-4 rounded-lg flex gap-3 items-start">
                  <AlertCircle className="w-5 h-5 text-red-400 shrink-0 mt-0.5" />
                  <div>
                    <h3 className="font-semibold text-red-200 text-sm">Error Detected</h3>
                    <p className="text-red-200/70 text-sm mt-1">{log.message}</p>
                    <p className="text-xs text-red-200/40 mt-2 font-mono">{new Date(log.createdAt!).toLocaleString()}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="bg-emerald-950/10 border border-emerald-500/20 p-8 rounded-lg flex flex-col items-center justify-center text-center">
              ...
              <h3 className="font-semibold text-emerald-200">All Systems Normal</h3>
              <p className="text-emerald-200/60 text-sm mt-1">No errors detected in the logs.</p>
            </div>
          )}
        </div>
      </div>
